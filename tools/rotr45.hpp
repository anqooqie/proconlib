...
namespace tools {
  ...
  std::array<std::array<T, N + M - 1>, N + M - 1> rotr45(const std::array<std::array<T, M>, N>& matrix, const T& default_value) {
    static_assert(N >= 1);
    static_assert(M >= 1);
  ...
  template <typename T>
  std::vector<std::vector<T>> rotr45(const std::vector<std::vector<T>>& matrix, const T& default_value) {
