#ifndef TOOLS_SUBSET_MOEBIUS_INPLACE_HPP
#define TOOLS_SUBSET_MOEBIUS_INPLACE_HPP
...
namespace tools {
  ...
  void subset_moebius_inplace(R&& b) {
    ...
    for (int w = 0; tools::pow2(w) < N; ++w) {
      for (int i = 0; i < N; i += tools::pow2(w)) {
        for (; !((i >> w) & 1); ++i) {
          std::ranges::begin(b)[i + tools::pow2(w)] = G::op(std::ranges::begin(b)[i + tools::pow2(w)], G::inv(std::ranges::begin(b)[i]));
        }
      }
    }
  }
  ...
  requires std::ranges::output_range<R, std::ranges::range_value_t<R>>
  void subset_moebius_inplace(R&& b) {
    tools::subset_moebius_inplace<tools::groups::plus<std::ranges::range_value_t<R>>>(std::forward<R>(b));
  }
}
