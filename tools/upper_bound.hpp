...
namespace tools {
  ...
  >
  constexpr ::std::iter_difference_t<I> upper_bound(const I first, const S last, const T& value, const Comp comp = {}, const Proj proj = {}) {
    return ::std::ranges::distance(first, ::std::ranges::upper_bound(first, last, value, comp, proj));
  }
  ...
  >
  constexpr ::std::ranges::range_difference_t<R> upper_bound(R&& r, const T& value, const Comp comp = {}, const Proj proj = {}) {
    return ::std::ranges::distance(::std::ranges::begin(r), ::std::ranges::upper_bound(r, value, comp, proj));
  }
}
