...
namespace tools {
  ...
  constexpr T rotl(const T x, const int n, const S s) noexcept {
    ...
    if (r == 0) return x;
    return ((y << r) & mask) | (y >> (n - r));
  }

  template <tools::non_bool_integral T, tools::non_bool_integral S>
  constexpr T rotl(const T x, const S s) noexcept {
    return tools::rotl(x, std::numeric_limits<T>::digits, s);
  }
  ...
  std::array<std::array<T, N>, M> rotl(const std::array<std::array<T, M>, N>& matrix) {
    std::array<std::array<T, N>, M> res;
    for (std::size_t r = 0; r < M; ++r) {
      for (std::size_t c = 0; c < N; ++c) {
        res[r][c] = matrix[c][M - 1 - r];
      }
    }
    return res;
  }
  ...
  template <typename T>
  std::vector<std::vector<T>> rotl(const std::vector<std::vector<T>>& matrix) {
  ...
  inline std::vector<std::string> rotl(const std::vector<std::string>& matrix) {
    const auto N = matrix.size();
    assert(N >= 1);
    const auto M = matrix.front().size();
    assert(M >= 1);
