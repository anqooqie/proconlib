...
## Constructor
```cpp
(1) avl_tree<M> avl_tree();
(2) avl_tree<M> avl_tree(int n);
(3) avl_tree<M> avl_tree(std::ranges::input_range auto&& v)
    requires std::assignable_from<S&, std::ranges::range_reference_t<R>>;
(4) avl_tree<M, Reversible> avl_tree();
(5) avl_tree<M, Reversible> avl_tree(int n);
(6) avl_tree<M, Reversible> avl_tree(std::ranges::input_range auto&& v)
    requires std::assignable_from<S&, std::ranges::range_reference_t<R>>;
```
...
- (1)
    - It is identical to `avl_tree<M, false> avl_tree();`.
- (2)
    - It is identical to `avl_tree<M, false> avl_tree(n);`.
- (3)
    - It is identical to `avl_tree<M, false> avl_tree(v);`.
- (4)
