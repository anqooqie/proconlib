...
std::array<std::array<T, N + M - 1>, N + M - 1> rotr45(const std::array<std::array<T, M>, N>& A, const T& default_value);
```
...
```cpp
template <typename T>
std::vector<std::vector<T>> rotr45(const std::vector<std::vector<T>>& A, const T& default_value);
```
