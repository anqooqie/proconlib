...
## Constructor
```cpp
(1) lazy_avl_tree<SM, FM, mapping> avl_tree();
(2) lazy_avl_tree<SM, FM, mapping> avl_tree(int n);
(3) lazy_avl_tree<SM, FM, mapping> avl_tree(std::ranges::input_range auto&& v)
    requires std::assignable_from<S&, std::ranges::range_reference_t<R>>;
(4) lazy_avl_tree<SM, FM, mapping, Reversible> avl_tree();
(5) lazy_avl_tree<SM, FM, mapping, Reversible> avl_tree(int n);
(6) lazy_avl_tree<SM, FM, mapping, Reversible> avl_tree(std::ranges::input_range auto&& v)
    requires std::assignable_from<S&, std::ranges::range_reference_t<R>>;
```
...
- (1)
    - It is identical to `lazy_avl_tree<SM, FM, mapping, false> avl_tree()`.
- (2)
    - It is identical to `lazy_avl_tree<SM, FM, mapping, false> avl_tree(n)`.
- (3)
    - It is identical to `lazy_avl_tree<SM, FM, mapping, false> avl_tree(v)`.
- (4)
