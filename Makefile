...
verify-multi-process: .competitive-verifier/local
	seq 0 $(shell expr $(shell nproc) - 1) | xargs -I {} -P $(shell nproc) bash -c 'competitive-verifier verify --verify-json .competitive-verifier/local/verify_files.json --prev-result .competitive-verifier/local/result.json --split $(shell nproc) --split-index {} --output .competitive-verifier/local/result-{}.json'
